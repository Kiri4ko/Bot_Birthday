...
async def quiz_handler(message: types.Message, state: FSMContext):
    await message.answer(
        "Правила ☝️:\n"
        "Я отправляю текст поздравления 🎁, а ты угадай \- кто автор❓\n"
        "Готова? 🤓👇",
...
async def q_2_handler(message: types.Message, state: FSMContext):
    if message.text[3:] == q_1.answer:
        ...
        await message.answer(escape_md("Да✅, Умница 😇"))
    else:
        await message.answer_sticker('CAACAgEAAxkBAAEGMVtjV7mzcmwQWp1MA0-M7CPafRlfAAMhAAM4DoIRwQjaTnufRQ4qBA')
        await message.answer(escape_md("Упс...Вот ответ 👉 ") + f"||{q_1.answer} 🥳||")

    await message.answer(f"*ПОЗДРАВЛЕНИЕ* 💌\-2:\n_{q_2.question}_",
...
async def q_3_handler(message: types.Message, state: FSMContext):
    if message.text[3:] == q_2.answer:
        ...
        await message.answer_sticker('CAACAgEAAxkBAAEGMVtjV7mzcmwQWp1MA0-M7CPafRlfAAMhAAM4DoIRwQjaTnufRQ4qBA')
        await message.answer(escape_md("Упс...Вот ответ 👉 ") + f"||{q_2.answer} 🥳||")

    await message.answer(f"*ПОЗДРАВЛЕНИЕ* 💌\-3:\n_{q_3.question}_",
...
async def q_4_handler(message: types.Message, state: FSMContext):
    if message.text[3:] == q_3.answer:
        ...
        await message.answer_sticker('CAACAgEAAxkBAAEGMVtjV7mzcmwQWp1MA0-M7CPafRlfAAMhAAM4DoIRwQjaTnufRQ4qBA')
        await message.answer(escape_md("Упс...Вот ответ 👉 ") + f"||{q_3.answer} 🥳||")

    await message.answer(f"*ПОЗДРАВЛЕНИЕ* 💌\-4:\n_{q_4.question}_",
...
async def q_5_handler(message: types.Message, state: FSMContext):
    if message.text[3:] == q_4.answer:
        ...
        await message.answer_sticker('CAACAgEAAxkBAAEGMVtjV7mzcmwQWp1MA0-M7CPafRlfAAMhAAM4DoIRwQjaTnufRQ4qBA')
        await message.answer(escape_md("Упс...Вот ответ 👉 ") + f"||{q_4.answer} 🥳||")

    await message.answer(f"*ПОЗДРАВЛЕНИЕ* 💌\-5:\n_{q_5.question}_",
...
async def q_6_handler(message: types.Message, state: FSMContext):
    if message.text[3:] == q_5.answer:
        ...
        await message.answer_sticker('CAACAgEAAxkBAAEGMVtjV7mzcmwQWp1MA0-M7CPafRlfAAMhAAM4DoIRwQjaTnufRQ4qBA')
        await message.answer(escape_md("Упс...Вот ответ 👉 ") + f"||{q_5.answer} 🥳||")

    await message.answer(f"*ПОЗДРАВЛЕНИЕ* 💌\-6:\n_{q_6.question}_",
...
async def q_7_handler(message: types.Message, state: FSMContext):
    if message.text[3:] == q_6.answer:
        ...
        await message.answer_sticker('CAACAgEAAxkBAAEGMVtjV7mzcmwQWp1MA0-M7CPafRlfAAMhAAM4DoIRwQjaTnufRQ4qBA')
        await message.answer(escape_md("Упс...Вот ответ 👉 ") + f"||{q_6.answer} 🥳||")

    gif = InputFile('static/media/daddy.gif')
    await message.answer_animation(animation=gif, caption=f"*ПОЗДРАВЛЕНИЕ* 💌\-7:\n_{q_7.question}_",
...
async def q_8_handler(message: types.Message, state: FSMContext):
    if message.text[3:] == q_7.answer:
        ...
        await message.answer_sticker('CAACAgEAAxkBAAEGMVtjV7mzcmwQWp1MA0-M7CPafRlfAAMhAAM4DoIRwQjaTnufRQ4qBA')
        await message.answer(escape_md("Упс...Вот ответ 👉 ") + f"||{q_7.answer} 🥳||")

    await message.answer(f"*ПОЗДРАВЛЕНИЕ* 💌\-8:\n_{q_8.question}_",
...
async def q_9_handler(message: types.Message, state: FSMContext):
    if message.text[3:] == q_8.answer:
        ...
        await message.answer_sticker('CAACAgEAAxkBAAEGMVtjV7mzcmwQWp1MA0-M7CPafRlfAAMhAAM4DoIRwQjaTnufRQ4qBA')
        await message.answer(escape_md("Упс...Вот ответ 👉 ") + f"||{q_8.answer} 🥳||")

    await message.answer(f"*ПОЗДРАВЛЕНИЕ* 💌\-9:\n_{q_9.question}_",
...
async def q_10_handler(message: types.Message, state: FSMContext):
    if message.text[3:] == q_9.answer:
        ...
        await message.answer_sticker('CAACAgEAAxkBAAEGMVtjV7mzcmwQWp1MA0-M7CPafRlfAAMhAAM4DoIRwQjaTnufRQ4qBA')
        await message.answer(escape_md("Упс...Вот ответ 👉 ") + f"||{q_9.answer} 🥳||")

    await message.answer(f"*ПОЗДРАВЛЕНИЕ* 💌\-10:\n_{q_10.question}_",
...
async def q_11_handler(message: types.Message, state: FSMContext):
    if message.text[3:] == q_10.answer:
        ...
        await message.answer_sticker('CAACAgEAAxkBAAEGMVtjV7mzcmwQWp1MA0-M7CPafRlfAAMhAAM4DoIRwQjaTnufRQ4qBA')
        await message.answer(escape_md("Упс...Вот ответ 👉 ") + f"||{q_10.answer} 🥳||")

    await message.answer(f"*ПОЗДРАВЛЕНИЕ* 💌\-11:\n_{q_11.question}_",
...
async def q_12_handler(message: types.Message, state: FSMContext):
    if message.text[3:] == q_11.answer:
        ...
        await message.answer_sticker('CAACAgEAAxkBAAEGMVtjV7mzcmwQWp1MA0-M7CPafRlfAAMhAAM4DoIRwQjaTnufRQ4qBA')
        await message.answer(escape_md("Упс...Вот ответ 👉 ") + f"||{q_11.answer} 🥳||")

    await message.answer(f"*ПОЗДРАВЛЕНИЕ* 💌\-12:\n_{q_12.question}_",
...
async def q_13_handler(message: types.Message, state: FSMContext):
    if message.text[3:] == q_12.answer:
        ...
        await message.answer_sticker('CAACAgEAAxkBAAEGMVtjV7mzcmwQWp1MA0-M7CPafRlfAAMhAAM4DoIRwQjaTnufRQ4qBA')
        await message.answer(escape_md("Упс...Вот ответ 👉 ") + f"||{q_12.answer} 🥳||")

    await message.answer(f"*ПОЗДРАВЛЕНИЕ* 💌\-13:\n_{q_13.question}_",
...
async def q_14_handler(message: types.Message, state: FSMContext):
    if message.text[3:] == q_13.answer:
        ...
        await message.answer_sticker('CAACAgEAAxkBAAEGMVtjV7mzcmwQWp1MA0-M7CPafRlfAAMhAAM4DoIRwQjaTnufRQ4qBA')
        await message.answer(escape_md("Упс...Вот ответ 👉 ") + f"||{q_13.answer} 🥳||")

    await message.answer(f"*ПОЗДРАВЛЕНИЕ* 💌\-14:\n_{q_14.question}_",
...
async def q_handler_finish(message: types.Message, state: FSMContext):
    if message.text[3:] == q_14.answer:
        ...
        await message.answer_sticker('CAACAgEAAxkBAAEGMVtjV7mzcmwQWp1MA0-M7CPafRlfAAMhAAM4DoIRwQjaTnufRQ4qBA')
        await message.answer(escape_md("Упс...Вот ответ 👉 ") + f"||{q_14.answer} 🥳||")

    await state.finish()
    await message.answer_sticker('CAACAgEAAxkBAAEGM_ZjWO7y5RqMlQItYItpWTkhyWczWgACLQADOA6CETXosuhOHHccKgQ')
    await message.answer(
        'Сюрприз 👉: [Кликни меня](https://olya-birthday.herokuapp.com)🎁'
        '\nЗапомни секретное слово ||Рица|| \- сообщай тем, кто попросит❗️'
        '\nИспользуй наушники \- так будет лучше❗️',
